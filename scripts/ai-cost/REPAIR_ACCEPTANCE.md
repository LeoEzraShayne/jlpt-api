...
One service round has two network slots. A retryable structural/language/evidence failure spends the second slot on one same-provider repair; an availability failure may spend it on a configured fallback. A repair replaces fallback, never adds a third call. Missing provider keys are skipped and cannot overwrite the real error. Nonretryable metering failures stop immediately. Grammar's existing three worker rounds remain at most six ordinary network attempts; each vocabulary generation/assessment has at most two. F subsequently reproduced that baseline stale-lease recovery could exceed this automatic budget. Main owns the atomic admission repair: automatic/crash recovery at most3 rounds/6 calls, plus the pre-existing explicit manual recovery of one extra round/2 calls, for lifecycle8. This corrects the original report's omission of manual recovery; the service evaluations here still use only2 calls per operation.
