...
New quotes snapshot `paymentMethodPolicy=CARD_ONLY_V1` and create Stripe Checkout with `payment_method_types: ['card']`. The first release accepts card payments, including eligible card wallets exposed by Stripe; other account-enabled methods are excluded so the supported payment methods match the cost assumptions. This setting applies only to this Checkout request, not other applications or the Stripe account configuration. Quotes created before this policy omit the parameter on retry, preserving their original provider idempotency hash; existing Checkout URLs are returned unchanged.
