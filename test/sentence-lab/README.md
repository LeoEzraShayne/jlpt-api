...
From a checkout containing W0, C's corrected corpus, and A's billing delivery:

```sh
DATABASE_URL=postgresql://localhost:5432/jlpt_f_acceptance_test_generation npm ci --no-audit --no-fund
export ACCEPTANCE_ORIGINAL_STATIC_SNAPSHOT=/absolute/path/to/original-public-static-content.json
...
`ACCEPTANCE_ORIGINAL_STATIC_SNAPSHOT` points to the preserved pre-correction snapshot; the correction tests require it separately. The generated output must not exist and must differ from the input. `ACCEPTANCE_STATIC_SNAPSHOT` points to the corrected copy and must be a public-only snapshot with exactly `GrammarPoint`, `GrammarExample`, `GrammarRelationGroup`, and `TrainingScenario`. The snapshot is intentionally external; it has production entity IDs and Japanese originals needed for independent source binding. It contains no user/account tables. Content tests execute the actual scenario-preparation and translation-import CLIs, with the test database URL and dotenv disabled.

For content-only verification after generating the corrected snapshot:

```sh
ACCEPTANCE_STATIC_SNAPSHOT=/tmp/f-corrected-static.json npx jest --config test/sentence-lab/jest.json --runInBand --runTestsByPath test/sentence-lab/content.acceptance-spec.ts
```
...
The separate explicitly invoked paid synthetic harness and independent cost calculations are documented in `ai-audit/REVIEW.md`; they are not run by Jest. `held-out.ts --run` refuses to replace existing evidence; a new authorized run needs a distinct `--tag=...` and retains the 12-network-call ceiling.
