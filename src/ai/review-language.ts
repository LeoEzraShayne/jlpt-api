...
export function localizedCoreReviewPrompt(input: ReviewProviderInput) {
  const language =
    input.explanationLocale === 'en' ? 'English' : 'Simplified Chinese';
...
Keep these legacy JSON field names, but ALL explanations, error reasons, translation and encouragement MUST be natural ${language}, even keys ending _zh. Japanese sentence/furigana fields stay Japanese. Do not translate quoted Japanese grammar forms.
Emit JSON keys in this order so the sum is computed from already finalized components: grammar_score, connection_score, completeness_score, naturalness_score, vocabulary_score, total_score, is_correct, used_target_grammar, target_grammar_correct, result_level, error_spans, corrected_sentence, corrected_sentence_furigana, corrected_sentence_translation_zh, corrected_sentence_uses_target_grammar, explanation_zh, encouragement, scenario_task_completed.
Maximum component scores are 30,20,20,20,10 respectively; Finalize the five integer components first, ensuring their sum respects the applicable target cap, then add them and write that EXACT sum as total_score. Never clamp only total_score while leaving a larger component sum. Missing target grammar caps total at 30; incorrect target grammar/connection caps at 59. Evaluate the original answer, not the correction. Read the exact target connection in the original character by character before scoring; never silently fix a conjugation and then mark the original correct. If the original connection is wrong, target_grammar_correct must be false, list that actual error and cap the total at 59. Award partial credit for comprehensible meaning, complete sentence structure and appropriate vocabulary even when the target connection is wrong; do not zero every dimension for one local error. Do not penalize simple natural sentences, repetition or absent optional vocabulary; correct simple sentences can score 100.
...
explanation_zh is one or two ${language} sentences describing the key error or correct usage; encouragement is one brief ${language} sentence.
