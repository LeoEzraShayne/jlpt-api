...
> 历史审查记录：下文记录 `630f058` 原型与当时缺口，不能作为当前操作 runbook。后续已采用匿名 User tombstone、数据库 guards 和账务专用路径修复，操作方法与剩余边界以 [最新安全交付说明](account-deletion-safety-delivery.md) 为准；当前测试断言也已转换为修复后的安全行为。
