...
## Crash recovery and subsequent model evaluation

F reproduced a crash window in the pre-existing worker: after provider receipts were written but before failure persistence, an expired lease reopened the same round and attempt indices. The main fix records round admission atomically when claiming a job, returns that immutable round with the lease and uses it for provider-call ordinals. An expired third automatic round is claimed only for terminal cleanup and quota release; no new provider call is made. Failure persistence no longer increments the counter. An old claimant that resumes after a newer lease keeps its original round's ordinals and cannot commit over the newer lease.

The existing explicit manual retry permits a fourth round. Preserve this recovery behavior: **up to six automatic calls, or eight total including the existing manual recovery**, at two calls per round. The earlier six-call statement describes automatic retries, not the lifetime including that manual request. The bound applies per review job, not across different authorized corrections in one study session. The existing retryCount column now records admitted rounds; no migration or learning quota change is needed. Old unmetered calls cannot be reconstructed retrospectively.

Main's real-PostgreSQL checks confirm crash exhaustion returns the reserved free quota, manual recovery reserves and releases that same submission without duplicating it, and no fifth round is admitted. F's previously failing seven-test repair suite now passes locally; independent recheck is pending.

D's optional thinking evaluation is integrated without enabling it by default. Full thinking improves the tested grammar semantics but still has strict-output failures and an estimated $439/year AI cost at 240 daily operations under the peak/cold scenario. This changes the economic evidence materially, including negative daily-pass contribution in that scenario. The earlier pricing question was based on the unqualified non-thinking route; final pricing must use a quality-qualified route. A grammar-thinking/vocabulary-non-thinking candidate and F's new frozen held-out set remain pending. No production configuration changed.
