...
An actual request from the backend server received HTTP 200 from Google OAuth using this key. The read-only voided-purchases call returned HTTP 403 `SERVICE_DISABLED` for `androidpublisher.googleapis.com`, project number `692705532429`. The API library page is ready at its Enable button, but explicitly states that use agrees to Google APIs Terms of Service and Google Play Android Developer API terms. A **new, separate terms-acceptance confirmation is pending**; the Enable button has not been clicked. This is a browser action-time legal-terms requirement, not a repeat request for the already approved service account/key/access. Gemini project billing was not enabled.
